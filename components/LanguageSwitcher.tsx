...
export function LanguageSwitcher({ variant = "panel", className }: LanguageSwitcherProps) {
  ...
  if (variant === "compact") {
    return (
      <div
        className={`flex items-center gap-2 text-[10px] text-slate-300 ${className ?? ""}`}
      >
        {supportedLanguages.map((value) => (
          <button
            ...
            className={`rounded-md border px-2 py-[2px] text-[10px] transition-colors ${
              language === value
  ...
  return (
    <div
      className={`flex flex-col gap-2 rounded-lg border border-slate-800 bg-slate-900/60 p-3 text-sm shadow-lg md:p-4 ${className ?? ""}`}
    >
